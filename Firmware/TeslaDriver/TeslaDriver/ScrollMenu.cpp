...
void ScrollMenu::PrintItem(int8_t item)
{
    ...
    if ( text != nullptr )
    {
        g_Display.Printf_P(text);
    }        
//    g_Display.WriteCharacter((m_SelectedItem == -1 && item == m_HighlightedItem) ? '\x7F' : ' ');
...
bool ScrollMenu::HandleEvent(const Event& event)
{
    switch(event.type)
    {
        case EventID::e_KeyDown:
            switch(event.value)
            {
                ...
                case KbdKeyCodes::e_Menu:
                    EventSys::AddEvent(Event(EventID::e_MenuSelected, m_HighlightedItem));
                    return true;
                case KbdKeyCodes::e_Left:
                    EventSys::AddEvent(Event(EventID::e_MenuSelected, -1));
                    return true;
            }
            break;
        case EventID::e_KnobTurned:
        {
            ...
            {
                EventSys::AddEvent(Event(EventID::e_MenuItemEdited, event.value));
            }
            return true;
        }            
        default:
